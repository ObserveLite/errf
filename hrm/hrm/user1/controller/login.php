<?php
	...
	else if (isset($_POST['usave']))
	{
		...
		else
		{
			...
			$sql = mysqli_query($db,"select * from user where Password='$old' and Email='$email'");
			if(mysqli_num_rows($sql) > 0)
			{
				if(!empty($npass) && !empty($cpass))
				{
					if($npass == $cpass)
					{
						mysqli_query($db,"update user set Password='$npass' where Password='$old' and Email='$email'");
	...
	else if(isset($_POST['submit']))
	{
		...
		{
			$sql = mysqli_query($db,"select * from user where Email='$Username' AND Password='$Password' AND RoleId=1 ");
			...
			if(mysqli_num_rows($sql) > 0)
			{
				...
				mysqli_query($db,"update user set LastLogin='$datetime' where Email='$email' ");
				$sqll = mysqli_query($db,"select * from user where RoleId='$roleid'");
	...
	function user()
	{
		...
		$mac = $GLOBALS["mac"];
		$sql = mysqli_query($db,"select * from user where Email='$Username' AND Password='$Password' AND StatusId=1 ");
