<?php
	...
	if(isset($_POST['clientlogin']))
	{
		...
		{
			header("location:../dean/index.php?msg=Username and Password is required");exit;
		}
		else
		{
	...
	else if (isset($_POST['usave']))
	{
		...
		if(empty($old))
		{
			...
			header("location:../dean/changepassword.php?msg=Old Password is required");exit;
		}
		else
		{

			$sql = mysqli_query($db,"select * from dean where Password='$old' and Email='$email'");
			if(mysqli_num_rows($sql) > 0)
			{
				if(!empty($npass) && !empty($cpass))
				{
					if($npass == $cpass)
					{
						mysqli_query($db,"update dean set Password='$npass' where Password='$old' and Email='$email'");
						header("location:../dean/changepassword.php?id=Successfull...");exit;	
					}
					else
					{
						header("location:../dean/changepassword.php?msg=New password and Confirm pssword do not match?");exit;
					}
					
				}
				else
				{
					header("location:../dean/changepassword.php?msg=New Password And Confirm Password is required?");exit;
				}
			}
			else
			{
				header("location:../dean/changepassword.php?msg=Old Password is Wrong!");exit;
			}
		}
	}
	...
	else if(isset($_POST['submit']))
	{
		...
		{
			$sql = mysqli_query($db,"select * from dean where Email='$Username' AND Password='$Password' AND RoleId=1 ");
			...
			if(mysqli_num_rows($sql) > 0)
			{
				...
				mysqli_query($db,"update dean set LastLogin='$datetime' where Email='$email' ");
	...
	function user()
	{
		...
		$mac = $GLOBALS["mac"];
		$sql = mysqli_query($db,"select * from dean where Email='$Username' AND Password='$Password' AND StatusId=1 ");
		...
		if(mysqli_num_rows($sql) > 0)
		{
			...
			if(mysqli_num_rows($ldate) > 0)
			{
				...
				header('location:../dean/home.php');exit;
			}
			else
			{
		...
		{
			header("location:../dean/index.php?msg=Username and Password is Wrong!");exit;
		}
	}
?>
